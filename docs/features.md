...
- **Lengkapi Profil** (`lib/app/complete_profile.dart`) — halaman wajib yang muncul setelah login (dari `login.dart` maupun restore sesi di `splash.dart`) kalau profil user belum lengkap. Trigger database `handle_new_user()` otomatis membuat baris `profiles` untuk user Google baru dengan `role = 'guru'` dan `alamat`/`no_telp` kosong (`''`); baris dengan salah satu dari keduanya kosong dianggap belum lengkap dan diarahkan ke sini alih-alih ke dashboard. User mengisi foto profil (kamera/galeri), `nama_lengkap`, `jabatan`, `alamat`, dan `no_telp` — semua wajib — lalu halaman melakukan `UPDATE` ke `profiles` (foto diunggah ke storage bucket `profiles`) dan baru kemudian meneruskan ke dashboard sesuai role. Halaman ini memblokir tombol back Android (`PopScope(canPop: false)`) selama profil belum lengkap; satu-satunya jalan keluar adalah tombol "Bukan Anda? Keluar" (sign-out).
- **Splash / restore sesi** — kalau sesi Supabase masih ada (mis. reload web setelah redirect OAuth), langsung diarahkan ke dashboard sesuai role tanpa perlu login ulang (kecuali profil belum lengkap, lihat poin di atas).
